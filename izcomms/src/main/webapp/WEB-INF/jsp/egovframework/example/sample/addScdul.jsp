	...
	<div>
		<form:form action="./addScdulPost.do" commandName="scdulVO" name="scdulForm" method="POST" onsubmit="return validateScdulForm(this)">
			<ul>
				<li>담당자</li>
				<li>
					<form:hidden path="memberGroup"/>
					<form:select path="member">
						<form:option value="">선택해주세요.</form:option>
						<c:forEach var="selectMemberList" items="${selectMemberList}">
							<form:option value="${selectMemberList.id}">${selectMemberList.name}</form:option>
						</c:forEach>
					</form:select>
				</li>
			</ul>
